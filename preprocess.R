...
save(sold, 'data/Rdata/sold.Rdata')

## Aggregate on median values per location (point)
sold$objectType <- as.numeric(sold$objectType)
sold.agg <- sold %>% group_by(longitude, latitude) %>% summarise_each(funs(median))
save(sold.agg, 'data/Rdata/sold-aggregate.Rdata')
